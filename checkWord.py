...
async def checkWords(l_word, words=words):
    ...
    word_lower = l_word.lower()
    word = await krill_latin(word_lower)
    matches = set(get_close_matches(word, words))
    available = False

    if word in matches:
        available = True
        matches = await krill_latin(word)
...
async def trans_matches(dictionary):
    ...
    trans = []
    for w in matche:
        soz = await krill_latin(f"✅ {w}")
        trans.append(soz)
    return trans


async def get_available(dictionary):
